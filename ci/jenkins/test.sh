...
function deliver_antrea_windows {
    ...
    curl -OL antrea-advanced-windows-v2.4.3_vmware.1.tar.gz
    mv antrea-advanced-windows-v2.4.3_vmware.1.tar.gz antrea-windows.tar.gz
...
if [[ ${TESTCASE} =~ "windows" ]]; then
    if [[ ${TESTCASE} == "windows-e2e-ovs-as-service" ]]; then
        WINDOWS_YAML_NAME="antrea-windows-v2.4.3+vmware.1"
    else
        WINDOWS_YAML_NAME="antrea-windows-v2.4.3+vmware.1"
    fi
