...
func (ic *ifConfigurator) addPostInterfaceCreateHook(containerID, endpointName string, containerAccess *containerAccessArbitrator, hook postInterfaceCreateHook) error {
	...
	go func() {
		...
		var err error
		pollErr := wait.PollUntilContextTimeout(context.TODO(), 100*time.Millisecond, 30*time.Second, true,
		...
		if pollErr != nil {
			if err != nil {
				...
				klog.ErrorS(pollErr, "Failed to wait for host interface creation in 30s", "interface", ifaceName)
			}
		}
	}()
	return nil
